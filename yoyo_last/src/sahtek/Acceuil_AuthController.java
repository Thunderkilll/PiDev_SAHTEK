  ...
  //  Services.Services_Patient sp = new Services_Patient();
    ...
    private void Authentifier(ActionEvent event) throws InterruptedException {
       ...
       if(su.authentifier(username ,passwd)){
         Parent test;
              try {
                 test = FXMLLoader.load(getClass().getResource("../Presentation/SplashSempei.fxml"));
                ...
                a.setTitle("Erreur de connection");
                a.setContentText("Votre mot de passe \nou votre username n'est pas correct \ns'il vous palit essaie encore une fois ou contacté l'admin");
                a.showAndWait().wait(500);
            }
    }else{
