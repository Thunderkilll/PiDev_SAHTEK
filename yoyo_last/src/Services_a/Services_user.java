...
public class Services_user implements IUsers{
    ...
    public Services_user()
    {
      ...
      public void addPropCentre(Users u) {
       String requete ="INSERT INTO `users` ( `username`, `nom`, `prenom`, `date_naissance`, `cin`, `email`, `password`, `telephone`, `adresse`, `user`,`id_centre`) VALUES (?,?, "
               + "?,?,?,?, ?,?,?,?,?);";
        ...
        ps.setInt(1, u.getCin());                                     ps.setString(5, u.getUsername());
        ps.setString(2, u.getNom());                                  ps.setString(3, u.getPrenom());
        ps.setString(6, u.getEmail());                                ps.setString(7, u.getPassword());
        ps.setDate(4,u.getDateNaissance() );                          ps.setInt(6, u.getTelephone());
        ps.setString(8, u.getAdresse());                               ps.setInt(10, u.getId_centre());
         ps.setString(9, u.getRole());      
        ps.executeUpdate();
        System.out.println("user ajouter Type : PropCentre");
           
        }
         catch (SQLException ex) {
             System.out.println(" error lors de l'ajout du propCentre"+ex);
        }
       
    }
     public Users recherchePatient(int cin) {
      Users p = new Users();
       try{
           Connection db = MyConnexion.Get_Instance().getCon();
              String req = "select * from `users` where `cin` = '"+cin+"'";
      ...
      //p.setImage(rs.getString(6));
           ...
           System.out.println("User "+p.getNom()+" Trouvé ");
       }catch(SQLException ex){
           System.out.println("erreur de requette recherche user "+ex);
       }
       return p;
    }
